...
mod_perform_df = MLJulFluxFun.mlj_mod_eval(forest_model, x, y, param_of_interest * "_" * model_name * "_training_performance_holdout", forest_model.n_trees, :ntree, 155:1:155, MLJulFluxFun.Holdout(eachindex(y), 0.8, true, 500));
# 5-fold cross-validation
mod_perform_df = MLJulFluxFun.mlj_mod_eval(forest_model, x, y, param_of_interest * "_" * model_name * "_training_performance_kfold", forest_model.n_trees, :ntree, 155:1:155, MLJulFluxFun.KFold(eachindex(y), 5, true, 100));
# Final trained model with tuned hyperparameters
mod_perform_df = MLJulFluxFun.mlj_mod_eval(forest_model, x, y, param_of_interest * "_" * model_name * "_trained_model", forest_model.n_trees, :ntree, 155:1:155);
...
# 20% holdout cross-validation
MLJulFluxFun.flux_mod_eval(nnet_build(nnet_mod_builder(N * 6, N * 4, N * 2), N, 1), x, y, param_of_interest * "_" * model_name * "_training_performance_holdout", MLJulFluxFun.Holdout(eachindex(y), 0.8, true, 500), 100, true, MLJulFluxFun.standard_scaler(), nothing, 5, trunc(Int64, round(J * 0.8)), 3, 2, Flux.Losses.mse, Flux.Optimise.ADAM(0.001));
# 5-fold cross-validation
MLJulFluxFun.flux_mod_eval(nnet_build(nnet_mod_builder(N * 6, N * 4, N * 2), N, 1), x, y, param_of_interest * "_" * model_name * "_trained_model", nothing, 500, false, MLJulFluxFun.standard_scaler(), nothing, 10, length(y), 3, 2, Flux.Losses.mse, Flux.Optimise.ADAM(0.001));
# Final trained model with tuned hyperparameters
